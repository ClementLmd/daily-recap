...
export const activitiesSlice = createSlice({
  ...
  reducers: {
    incrementCount: (state, action) => {
      const activity = state.activities.find((act) => act._id === action.payload);
      if (activity) {
    ...
    setCount: (state, action) => {
      const activity = state.activities.find((act) => act._id === action.payload.activityId);
      if (activity) {
    ...
    builder
      ...
      .addCase(deleteProgress.fulfilled, (state, action) => {
        ...
        if (updatedActivity && updatedActivity._id) {
          const activity = state.activities.find((act) => act._id === updatedActivity._id);
          if (activity) {
